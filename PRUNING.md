...
- **Below-Threshold Skipping (`skipped-below-threshold`):** When `minRawCharThreshold` is set above 0, batches whose raw tool-output is below the threshold never even reach the summarizer — no LLM call is made. This prevents the exact situation above (a summary larger than the raw text) from happening for tiny tool results. The frontier still advances so those turns are not retried forever.
